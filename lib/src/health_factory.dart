...
class HealthFactory {
  ...
  bool isDataTypeAvailable(HealthDataType dataType) =>
      _platformType == PlatformType.ANDROID ? _dataTypeKeysAndroid.contains(dataType) : _dataTypeKeysIOS.contains(dataType);
  ...
  Future<List<HealthDataPoint>> _computeAndroidBMI(DateTime startDate, DateTime endDate) async {
    ...
    for (var i = 0; i < weights.length; i++) {
      ...
      final x = HealthDataPoint(bmiValue, dataType, unit, weights[i].dateFrom, weights[i].dateTo, _platformType, _deviceId!, '', '');
